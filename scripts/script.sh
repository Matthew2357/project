...
for ((seed=1; seed <= runs; seed++)); do
    ...
    python -W ignore ./src/main.py --seed $seed --wandb_project "$wandb_project" --wandb_group "$method" --trust_freq 25 --pretraining_rounds 100 --iterations 500 --num_clients $num_clients --eval_freq 25 --method "$method" --dataset $dataset_name --wandb --config_format lora --use_pretrained gpt2 --lora_mlp --lora_causal_self_attention --lora_freeze_all_non_lora 
done
